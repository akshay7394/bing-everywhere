...
(function () {
    // Execute the script only on Google Search result pages
    if (!window.location.href.startsWith('https://www.google.com/search')) {
        return;
    }

    // Wait until the page has fully loaded before executing the script
    ...
    function injectChatIframe() {
        // Create a new iframe element
        const iframe = document.createElement('iframe');
        iframe.id = 'underside-iframe-container';
        iframe.name = 'underside-iframe-container';
        iframe.src = 'https://edgeservices.bing.com/edgediscover/query?&lightschemeovr=1&FORM=SHORUN&udscs=1&udsnav=1&setlang=en-US&features=udssydinternal&clientscopes=windowheader,coauthor,chat,&udsframed=1';
        ...
        iframe.src = 'https://edgeservices.bing.com/edgediscover/query?&darkschemeovr=1&FORM=SHORUN&udscs=1&udsnav=1&setlang=en-US&features=udssydinternal&clientscopes=windowheader,coauthor,chat,&udsframed=1';
        }
        ...
        // Select the input field in the chatbot iframe so the user can input a search query
        iframe.onload = function () {
            const input = iframe.contentWindow.document.querySelector('input[name="q"]');
        };
